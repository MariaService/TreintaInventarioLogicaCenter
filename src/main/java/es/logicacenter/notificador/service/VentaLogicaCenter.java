		...
		try {
			...
			for (Transaction transaction : response.getTransactions()) {
				...
				if (isExiteFolio == 0) {
					...
					double totalOtraTiendaVenta = SumaMontoTotal(Tienda.BRAVO.getValor(),
							EnumOperacionTienda.OPERACION_VENTA.getValor());

					double totalOtraTiendaGasto = SumaMontoTotal(Tienda.BRAVO.getValor(),
							EnumOperacionTienda.OPERACION_GASTO.getValor());

					// -------------------- mensaje de gasto tienda.
					String msjeVentDescripcion = soloFecha + "💰 Lógica Center Morelos " + transaction.getDescription()
					...
					mensajeNotifiador.setNombreTienda(" 🏪 Lógica Center Morelos");
					...
					mensajeNotifiador.setListEgresos(
							getListaGasto(Tienda.MORELOS.getValor(), EnumOperacionTienda.OPERACION_GASTO.getValor()));
					...
					mensajeNotifiador.setCatTienda(Tienda.BRAVO.getValor());
